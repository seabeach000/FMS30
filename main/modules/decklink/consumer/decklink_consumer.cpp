	...
	virtual HRESULT STDMETHODCALLTYPE ScheduledFrameCompleted(IDeckLinkVideoFrame* completed_frame, BMDOutputFrameCompletionResult result)
	{
		...
		try
		{
			...
			if(result == bmdOutputFrameDisplayedLate)
			{
				...
				audio_scheduled_ += format_desc_.audio_sample_rate / format_desc_.fps;//上面这句话audio 的size可能不正确，是一个比较大的值，从而造成audio_scheduled值很大，所以decklink会卡住不动，所以更新为下面的语句通过sample rate和视频帧率来计算
