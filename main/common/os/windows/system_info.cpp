...
std::wstring cpu_info()
{
	...
	if(RegOpenKeyEx(HKEY_LOCAL_MACHINE, L"HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0", 0, KEY_QUERY_VALUE, &hkey) == ERROR_SUCCESS)
	{
		...
		if(RegQueryValueEx(hkey, L"ProcessorNameString", NULL, &dwType, (PBYTE)&p_name_str, &dwSize) == ERROR_SUCCESS)		
			cpu_name = p_name_str;		
...
std::wstring system_product_name()
{
	...
	if(RegOpenKeyEx(HKEY_LOCAL_MACHINE, L"HARDWARE\\DESCRIPTION\\System\\BIOS", 0, KEY_QUERY_VALUE, &hkey) == ERROR_SUCCESS)
	{
		...
		if(RegQueryValueEx(hkey, L"SystemProductName", NULL, &dwType, (PBYTE)&p_name_str, &dwSize) == ERROR_SUCCESS)		
...
std::wstring win_product_name()
{
	...
	if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion", 0, KEY_QUERY_VALUE, &hkey) == ERROR_SUCCESS)
	{
		...
		if (RegQueryValueEx(hkey, L"ProductName", NULL, &dwType, (PBYTE)&p_name_str, &dwSize) == ERROR_SUCCESS)
			result = p_name_str;
...
std::wstring win_sp_version()
{
	...
	if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion", 0, KEY_QUERY_VALUE, &hkey) == ERROR_SUCCESS)
	{
		...
		if (RegQueryValueEx(hkey, L"CSDVersion", NULL, &dwType, (PBYTE)&csd_ver_str, &dwSize) == ERROR_SUCCESS)
			result = csd_ver_str;
