...
private:
	...
	void do_show(bool value)
	{
		if(value)
		{
			if(!window_)
			{
				window_.reset(new sf::RenderWindow(sf::VideoMode(RENDERING_WIDTH, RENDERING_WIDTH), "Famous Diagnostics"));
...
public:
	...
	void render(sf::RenderTarget& target, sf::RenderStates states)
	{
		...
		for (auto& tag : line_tags_)
		{
			if (tag)
			{
				(*tag)[std::size_t(0)].position.x -= x_delta_;
				(*tag)[std::size_t(1)].position.x -= x_delta_;
			}
		}
