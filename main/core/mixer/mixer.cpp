		...
		{		
			try
			{
				...
				for (auto& frame : frames)
				{
					...
					if (afd_cmd.afd_aspect_ratio_ == -1 && afd_cmd.afd_command_ == -1)
					{
						afd_cmd = frame.second.out_afd_command;
					}
					if (src_afd_msg == -1)
					{
						src_afd_msg = frame.second.source_afd_code;//src command.
					}
				}
				...
				return const_frame(std::move(image), std::move(audio), this, desc, channel_layout,afd_cmd,src_afd_msg);
			}
			catch(...)
			{
