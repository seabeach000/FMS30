...
namespace caspar { namespace IO {
	...
	typedef std::function<std::pair<std::wstring, std::shared_ptr<void>> (const std::string& ipv4_address)>
		serial_lifecycle_factory_t;

class AsyncEventServer_Serial : boost::noncopyable
{
public:
	explicit AsyncEventServer_Serial(
			std::shared_ptr<boost::asio::io_service> service, const protocol_strategy_factory<char>::ptr& protocol, unsigned short port);
	~AsyncEventServer_Serial();

	void add_client_lifecycle_object_factory(const serial_lifecycle_factory_t& lifecycle_factory);
