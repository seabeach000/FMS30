...
VDCPCommand::ptr_type find_command(
		const std::map<std::wstring, std::pair<vdcp_command_func, int>>& commands,
		...
		std::list<std::wstring>& tokens)
{
	...
	/*if (!tokens.empty())
	{
		...
		{
			tokens.pop_front();
			return std::make_shared<VDCPCommand>(ctx, subcmd->second.first, subcmd->second.second, s);
		}
	}*/
	...
	if (command != commands.end())
		return std::make_shared<VDCPCommand>(ctx, command->second.first, command->second.second, s);

	return nullptr;
}

struct vdcp_command_repository::impl
{
	...
	std::map<std::wstring, std::pair<vdcp_command_func, int>>	commands;
	std::map<std::wstring, std::pair<vdcp_command_func, int>>	channel_commands;
...
vdcp_command_repository::vdcp_command_repository(
		const std::vector<spl::shared_ptr<core::video_channel>>& channels,
		const std::shared_ptr<core::thumbnail_generator>& thumb_gen,
		const spl::shared_ptr<core::media_info_repository>& media_info_repo,
		const spl::shared_ptr<core::system_info_provider_repository>& system_info_provider_repo,
...
VDCPCommand::ptr_type vdcp_command_repository::create_command(const std::wstring& s, IO::ClientInfoPtr client, std::list<std::wstring>& tokens) const
{
	auto& self = *impl_;
...
const std::vector<channel_context>& vdcp_command_repository::channels() const
{
	return impl_->channels;
}

VDCPCommand::ptr_type vdcp_command_repository::create_channel_command(
		const std::wstring& s,
...
void vdcp_command_repository::register_command(
		std::wstring category,
		std::wstring name,
		core::help_item_describer describer,
		vdcp_command_func command,
		int min_num_params)
{
	auto& self = *impl_;
	self.help_repo->register_item({ L"VDCP", category }, name, describer);
	self.commands.insert(std::make_pair(std::move(name), std::make_pair(std::move(command), min_num_params)));
}

void vdcp_command_repository::register_channel_command(
		std::wstring category,
		std::wstring name,
		core::help_item_describer describer,
		vdcp_command_func command,
