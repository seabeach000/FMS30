...
std::map<std::wstring, VDCPCommandQueue*>& get_instances()
{
	static std::map<std::wstring, VDCPCommandQueue*> queues;
...
VDCPCommandQueue::VDCPCommandQueue(const std::wstring& name)
	: executor_(L"VDCPCommandQueue " + name)
{
...
VDCPCommandQueue::~VDCPCommandQueue()
{
	tbb::spin_mutex::scoped_lock lock(get_global_mutex());
...
void VDCPCommandQueue::AddCommand(VDCPCommand::ptr_type pCurrentCommand)
{
	if(!pCurrentCommand)
		return;
	...
	if(executor_.size() > 128)
	{
		try
		{
			CASPAR_LOG(error) << "VDCP Command Queue Overflow.";
...
boost::property_tree::wptree VDCPCommandQueue::info() const
{
...
boost::property_tree::wptree VDCPCommandQueue::info_all_queues()
{
	boost::property_tree::wptree info;
	tbb::spin_mutex::scoped_lock lock(get_global_mutex());
